...
from functions_forecastv6 import *
from functions_cleanv9 import *
from helpv4 import *
...
label_version = 'v.1.0.3 30042021'
...
def file_output(key):  # Exportação do dataset
    ...
    messagebox.showinfo('Exportado', 'Dados exportados')
...
class MainGUI(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.title("WISDom - Validação de séries temporais de caudal" + "    (" + label_version + ")")  # Título
        ...
        for F in (Tab0, Tab1, Tab2, Tab3, Tab4):
            frame = F(container)  # Inicia construtor do separador
            self.frames[F] = frame
            frame.grid(row=0, column=0, sticky="nsew")

        self.show_frame(Tab0, from_menu=True)  # Default - Capa do projeto
    ...
    def file_input(self, tab):  # Introdução do ficheiro
        ...
        tk.messagebox.showinfo('Carregado', 'Dados carregados')
    ...
    def history_input(self):  # Introdução do ficheiro
        ...
        if dialect is True:
            dataset = pd.read_csv(file_inp, header=0, names=['date', 'value'], parse_dates=['date'],
                                  infer_datetime_format=True).sort_values(by=['date']).reset_index(drop=True)
        else:  # Apenas coloca cabeçalho uniforme
            dataset = pd.read_csv(file_inp, names=['date', 'value'], parse_dates=['date'],
                                  infer_datetime_format=True).sort_values(by=['date']).reset_index(drop=True)
        ...
        tk.messagebox.showinfo('Carregado', 'Histórico carregado')

    def show_frame(self, page_name, from_menu=False):
        """Show a frame for the given page name"""
        frame = self.frames[page_name]  # Apanha a frame requirida
        frame.tkraise()  # Coloca-a "à frente" da plataforma
        if not from_menu:  # Se o separador não foi selecionado pelo menubar
            frame.tab_functions()
...
class Tab0(tk.Frame):  # Capa
    def __init__(self, parent):
        ...
        self.background = PhotoImage(file="background_cover2.png")
        self.canvas.create_image(0, 0, image=self.background, anchor="nw")

        self.img0 = PhotoImage(file="Wisdom_logotipo2.png")
        ...
        self.img3 = PhotoImage(file="inesc3.png")
        self.canvas.create_image(1100/5 * 3, 600, image=self.img3, anchor="center")

        self.img4 = PhotoImage(file="fct2.png")
        self.canvas.create_image(1100/5 * 4, 600, image=self.img4, anchor="center")
        
        self.canvas.create_text(1100/2, 400, fill="white", font="Helvetica 14 bold",
                                text="Ferramenta para validação de séries temporais de caudal")
        self.canvas.create_text(1050, 20, fill="white", font="Helvetica 8",
                                text=label_version)
        
        # character = PhotoImage(file="ist.png")
        # canvas.create_image(1,1,image=character)
        # # Preparação de uma grelha 100 x 100 (colunas x linhas) com grossura de 1
        # for x in range(100):
        #     self.columnconfigure(x, weight=1)
        # for x in range(100):
        ...
        # # Preparação da capa
        # #### Background ####
        # img0 = tk.PhotoImage(file="background_cover2.png")
        # img0_label = tk.Label(self, image=img0, bg="white")
        # img0_label.image = img0
        # img0_label.place(x=0, y=0, relwidth=1, relheight=1)

        # #### Logótipo ####
        # img1 = tk.PhotoImage(file="logo.png")
        # img1_label = tk.Label(self, image=img1, bg="white")
        # img1_label.image = img1
        # img1_label.grid(row=5, column=54, sticky='NSEW')

        # #### Título da plataforma ####
        # tk.Label(self, text='Ferramenta para validação de séries temporais de caudal', bg='white',
        #          font='Helvetica 11 bold').grid(row=6, column=54, sticky='NSEW')
        # tk.Label(self, text=label_version, bg='white').grid(row=0, column=70, columnspan=30, sticky='NSEW')

        # ####--------Parceiros--------####
        # #### Instituto Superior Técnico ####
        # img2 = tk.PhotoImage(file="ist.png")
        # img2_label = tk.Label(self, image=img2, bg="white")
        # img2_label.image = img2
        # img2_label.grid(row=12, column=30, sticky='NSEW')

        # #### Instituto Politécnico de Setúbal ####
        # img3 = tk.PhotoImage(file="ips.png")
        # img3_label = tk.Label(self, image=img3, bg="white")
        # img3_label.image = img3
        # img3_label.grid(row=12, column=53, sticky='NSEW')

        # #### Instituto de Engenharia de Sistemas e Computadores - Investigação e Desenvolvimento ####
        # img4 = tk.PhotoImage(file="inesc.png")
        # img4_label = tk.Label(self, image=img4, bg="white")
        # img4_label.image = img4
        # img4_label.grid(row=12, column=55, sticky='NSEW')

        # #### Fundação de Ciência e Tecnologia
        # img5 = tk.PhotoImage(file="fct.png")
        # img5_label = tk.Label(self, image=img5, bg="white")
        # img5_label.image = img5
        # img5_label.grid(row=12, column=69, sticky='NSEW')
    ...
    def menubar(self, root):  # Menu - Semelhante em cada separador, diferenciando na visualização dos gráficos (WIP)
        ...
        fileMenu.add_command(label="Importar", command=lambda: root.file_input(Tab0))
        ...
        methodMenu.add_command(label="Identificação de falhas", command=lambda: root.show_frame(Tab1, from_menu=True))
        methodMenu.add_command(label="Imputação de valores pontuais",
                               command=lambda: root.show_frame(Tab2, from_menu=True))
        methodMenu.add_command(label="Normalização temporal", command=lambda: root.show_frame(Tab3, from_menu=True))
        methodMenu.add_command(label="Preenchimento de falhas", command=lambda: root.show_frame(Tab4, from_menu=True))
        menubar.add_cascade(label="Métodos de tratamento", menu=methodMenu)

        helpMenu = tk.Menu(menubar, tearoff=False)  # Ajuda e Sobre
        helpMenu.add_command(label="Ajuda", command=help_menu)
        helpMenu.add_command(label="Sobre", command=about_dsc)
...
class Tab1(tk.Frame):  # 1- Identificação de falhas
    def __init__(self, parent):
        ...
        tk.Button(self, text="Remover anómalos", command=self.remove_all, font=NORMAL_FONT,
                  relief=GROOVE).grid(row=68, column=75, columnspan=10, sticky='NSEW')

        # Avançar
        tk.Button(self, text="Avançar", command=self.next_tab, font=NORMAL_FONT,
                  relief=GROOVE).grid(row=90, column=75, columnspan=10, sticky='NSEW')

        # Inputs de parâmetros
        #### Picos altos ####
        # Tamanho da janela
        tk.Label(self, text="Janela (s)", bg='white',
        ...
        # Threshold
        tk.Label(self, text="Threshold (m\N{SUPERSCRIPT THREE}/h/s)", bg='white',
        ...
        # Threshold
        tk.Label(self, text="Threshold (m\N{SUPERSCRIPT THREE}/h)", bg='white',
    ...
    def menubar(self, root):  # O mesmo com o Tab0, adicionando os gráficos respetivos
        ...
        fileMenu = tk.Menu(menubar, tearoff=False)  # Ficheiro
        fileMenu.add_command(label="Importar", command=lambda: root.file_input(Tab1))
        fileMenu.add_command(label="Limpar dados", underline=1, command=self.clear)
        fileMenu.add_command(label="Exportar", underline=1, command=lambda: file_output("dataset1"))
        fileMenu.add_separator()
        fileMenu.add_command(label="Sair", command=self.quit)
        menubar.add_cascade(label="Ficheiro", menu=fileMenu)
        ...
        graphMenu.add_command(label="Série original", command=self.G1)
        graphMenu.add_command(label="Picos anómalos altos", command=self.G2)
        graphMenu.add_command(label="Picos anómalos baixos", command=self.G3)
        graphMenu.add_command(label="Patamares estáticos", command=self.G4)
        graphMenu.add_command(label="Série com anómalos identificados", command=self.G5)
        menubar.add_cascade(label="Visualizar gráficos", menu=graphMenu)
    ...
    def graph_original(self):  # Gráfico da Série original, em branco
        ...
        self.ax.set_title("Série original")

        # Label das horas
        self.ax.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                     ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax.legend()

        self.ax.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')  # Label do Y (caudal m^3/h)
    ...
    def upd_graph_original(self, values, dates):  # Atualiza gráfico da série original
        ...
        self.ax.plot(dates, values, '#4772FF', label='Caudal')
    ...
    def table_original(self):  # Estatisticas do Input
        tk.Label(self, text="Análise do espaçamento", bg='white', font=NORMAL_FONT,
                 wraplength=100).grid(column=50, row=0, columnspan=5, rowspan=1, sticky='NSWE')
        tk.Label(self, text="Análise do caudal", bg='white', font=NORMAL_FONT,
                 wraplength=80).grid(column=55, row=0, columnspan=5, rowspan=1, sticky='NSWE')
        tk.Label(self, text="Contagem", bg='white', font=NORMAL_FONT).grid(
            column=45, row=1, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Média", bg='white', font=NORMAL_FONT).grid(
            column=45, row=2, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Dsv.Pdr", bg='white', font=NORMAL_FONT).grid(
            column=45, row=3, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Mínimo", bg='white', font=NORMAL_FONT).grid(
        ...
        tk.Label(self, text="Máximo", bg='white', font=NORMAL_FONT).grid(
    ...
    def histograms_original(self):  # Histogramas do Input
        ...
        self.hist.set_title("Histograma Espaçamento")
        ...
        self.hist2.set_title("Histograma Caudal")
    ...
    def peak_high_original(self):  # Picos altos
        ...
        self.deltaq_pho.set_title("Picos anómalos altos")

        # Labels e gráfico default para gráfico do caudal
        self.deltaq_pho.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                             ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label="Diferença de Caudal")
        self.deltaq_pho.set_ylabel(r'Diferença de Caudal ($m^{3}h^{-1}$)')
        ...
        self.gradq_pho.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                            ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label="Gradiante de Caudal")

        self.gradq_pho.set_ylabel(r'Gradiante de Caudal ($m^{3}/h/s$)')
    ...
    def upd_peak_high(self, df, high_peaks, flow_PH, slopes_PH):  # Atualiza gráficos dos picos altos
        ...
        # Gráfico do caudal
        self.deltaq_pho.plot(df['date'], flow_PH, "#6F8EAF", label="Diferença de Caudal")
        self.deltaq_pho.plot(df['date'][high_peaks], flow_PH[high_peaks], "^r", label="Picos altos identificados")
        ...
        self.gradq_pho.plot(df['date'], slopes_PH, "#6F8EAF", label="Gradiante de Caudal")
        self.gradq_pho.plot(df['date'][high_peaks], slopes_PH[high_peaks],
                            "^r", label="Picos altos identificados")
    ...
    def peak_low_original(self):  # Picos Baixos, Igual aos picos altos
        ...
        self.deltaq_plo.set_title("Picos anómalos baixos")
        self.deltaq_plo.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                             ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label="Diferença de Caudal")
        self.deltaq_plo.set_ylabel(r'Diferença de Caudal ($m^{3}h^{-1}$)')
        ...
        self.gradq_plo.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                            ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label="Gradiante de Caudal")
        self.gradq_plo.set_ylabel(r'Gradiante de Caudal ($m^{3}/h/s$)')
    ...
    def upd_peak_low(self, df, low_peaks, flow_PL,
        ...
        self.deltaq_plo.plot(df['date'], flow_PL, "#6F8EAF", label="Diferença de Caudal")
        self.deltaq_plo.plot(df['date'][low_peaks], flow_PL[low_peaks], "^m", label="Picos baixos identificados")
        ...
        self.gradq_plo.plot(df['date'], slopes_PL, "#6F8EAF", label="Gradiante de Caudal")
        self.gradq_plo.plot(df['date'][low_peaks], slopes_PL[low_peaks], "^m", label="Picos baixos identificados")
        self.gradq_plo.axhline(y=float(self.tPL.get()), linestyle="--", label="Threshold")
    ...
    def upd_flat(self, df, temp_df, flat_points):  # Atualiza gráficos dos patamares
        ...
        self.flat.plot(df['date'], df['value'], "#6F8EAF", label="Caudal")  # Plot original
        self.flat.plot(temp_df['date'][flat_points], temp_df['value'][flat_points], "^g", label="Pontos em patamar")
    ...
    def final_graph(self):  # Série de Output
        ...
        self.final_fig.set_title("Série com anómalos identificados")

        # we plot y as a function of a, which parametrizes x
        self.final_fig.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                            ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.final_fig.legend()
        self.final_fig.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_final_graph(self, df, temp_df, negative_peaks, high_peaks, low_peaks,
        ...
        # Gráfico original
        self.final_fig.plot(df['date'], df['value'], '#4772FF', label='Caudal')

        # Negativos
        self.final_fig.plot(temp_df['date'][negative_peaks], temp_df['value'][negative_peaks],
                            linestyle='None', marker="x", color='#D95221', label="Negativos")

        # Picos altos
        self.final_fig.plot(temp_df['date'][high_peaks], temp_df['value'][high_peaks], "xr", label="Picos altos")

        # Picos Baixos
        self.final_fig.plot(temp_df['date'][low_peaks], temp_df['value'][low_peaks], "xm", label="Picos baixos")

        # Patamares
        self.final_fig.plot(temp_df['date'][flat_points], temp_df['value'][flat_points], "xg", label="Patamares")
    ...
    def table_results_original(self):  # Tabela resultados finais
        tk.Label(self, text="Contagem", bg='white', font=NORMAL_FONT).grid(column=80, row=73, sticky='NSWE')
        tk.Label(self, text="Dupl. Exact", bg='white', font=NORMAL_FONT).grid(column=79, row=74, sticky='NSWE')
        tk.Label(self, text="Dupl. Diff", bg='white', font=NORMAL_FONT).grid(column=79, row=75, sticky='NSWE')
        tk.Label(self, text="Negativos", bg='white', font=NORMAL_FONT).grid(
            column=79, row=76, sticky='NSWE')
        tk.Label(self, text="Picos altos", bg='white', font=NORMAL_FONT).grid(
            column=79, row=77, sticky='NSWE')
        tk.Label(self, text="Picos baixos", bg='white', font=NORMAL_FONT).grid(
            column=79, row=78, sticky='NSWE')
        tk.Label(self, text="Patamares", bg='white', font=NORMAL_FONT).grid(
            column=79, row=79, sticky='NSWE')
        tk.Label(self, text="Leituras nulas", bg='white', font=NORMAL_FONT).grid(
            column=79, row=80, sticky='NSWE')
        tk.Label(self, text="Total", bg='white', font=NORMAL_FONT).grid(
    ...
    def G1(self):  # Série Original
        try:
            ...
            plt.figure(1)
            plt.cla()
            plt.plot(df['date'], df['value'], '#4772FF', label='Caudal')
            plt.gca().xaxis.set_major_formatter(DATE_FORM)
            plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def G2(self):  # Picos altos
        ...
        plt.plot(df['date'][peaks['high_peaks']], temp_df['value'][peaks['high_peaks']], "xr",
                 label="Picos altos identificados")  # Plotting of all peaks
        plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
        plt.title("Picos anómalos altos")
        plt.legend()

        # Plot the differences in the values
        plt.subplot(3, 1, 2)
        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.plot(df['date'], peaks['flow_PH'], "#6F8EAF", label="Diferença de Caudal")
        plt.plot(df['date'][peaks['high_peaks']], peaks['flow_PH'][peaks['high_peaks']], "^r",
                 label="Picos altos identificados")
        plt.ylabel(r'Diferença de Caudal ($m^{3}h^{-1}$)')
        plt.legend()

        # Plot the slopes along with the threshold
        plt.subplot(3, 1, 3)
        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.plot(df['date'], peaks['slopes_PH'], "#6F8EAF", label="Gradiante de Caudal")
        ...
        plt.ylabel(r'Gradiante de Caudal ($m^{3}/h/s$)')
        plt.legend()
    ...
    def G3(self):  # Picos baixos
        ...
        plt.plot(df['date'][peaks['low_peaks']], temp_df['value'][peaks['low_peaks']], "xm",
                 label="Picos baixos identificados")  # Plotting of all peaks
        plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
        plt.title("Picos anómalos baixos")
        plt.legend()

        # Plot the differences in the values
        plt.subplot(3, 1, 2)
        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.plot(df['date'], peaks['flow_PL'], "#6F8EAF", label="Diferença de Caudal")
        plt.plot(df['date'][peaks['low_peaks']], peaks['flow_PL'][peaks['low_peaks']], "^m",
                 label="Picos baixos identificados")
        plt.ylabel(r'Diferença de Caudal ($m^{3}h^{-1}$)')
        plt.legend()
        ...
        plt.subplot(3, 1, 3)
        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.plot(df['date'], peaks['slopes_PL'], "#6F8EAF", label="Gradiante de Caudal")
    ...
    def G4(self):  # Patamares
        ...
        plt.plot(df['date'], df['value'], '#6F8EAF', label='Caudal')
        plt.plot(df['date'][peaks['flat_points']], temp_df['value'][peaks['flat_points']], "^g",
                 label="Pontos em patamar")
        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
        plt.title("Patamares estáticos")
        plt.legend()
        plt.show()
    ...
    def G5(self):  # Série sem falhas
        ...
        plt.plot(df['date'][peaks['negative_peaks']], temp_df['value'][peaks['negative_peaks']], linestyle='None',
                 marker="x", color='#D95221', label="Negativos")
        plt.plot(df['date'][peaks['high_peaks']], temp_df['value'][peaks['high_peaks']], "xr", label="Picos altos")
        plt.plot(df['date'][peaks['low_peaks']], temp_df['value'][peaks['low_peaks']], "xm", label="Picos baixos")
        plt.plot(df['date'][peaks['flat_points']], temp_df['value'][peaks['flat_points']], "xg", label="Patamares")

        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
        plt.title("Série com anómalos identificados")
        plt.legend()
        plt.show()
...
class Tab2(tk.Frame):  # Imputação dos valores pontuais
    def __init__(self, parent):
        ...
        # Buttons
        self.Btn_Sep = tk.Button(self, text="Separar falhas de curta e longa duração",
                                 command=self.upd_graph_nas_separated,
                                 font=NORMAL_FONT, relief=GROOVE).grid(column=92, row=5, rowspan=2, columnspan=6,
                                                                       sticky='NSWE')
        self.Btn_imputation = tk.Button(self, text="Imputação de valores para falhas de curta duração",
                                        command=self.imputation,
                                        font=NORMAL_FONT, relief=GROOVE).grid(column=90, row=45, columnspan=9,
                                                                              sticky='NSWE')
        self.Btn_Next = tk.Button(self, text="Avançar", command=self.next_tab,
                                  font=NORMAL_FONT, relief=GROOVE).grid(row=90, column=92, columnspan=4,
                                                                        sticky='NSEW')
        self.Btn_Help = tk.Button(self, text="Ajuda", command='',
        ...
        self.imputation_box['values'] = ('Interpolação Linear',
    ...
    def menubar(self, root):  # Menu, igual ao Tab 1, só muda os gráficos a visualizar (WIP)
        ...
        fileMenu.add_command(label="Importar", command=lambda: root.file_input(Tab2))
        fileMenu.add_command(label="Limpar dados", underline=1, command=self.clear)
        fileMenu.add_command(label="Exportar", underline=1, command=lambda: file_output("dataset2"))
        fileMenu.add_separator()
        fileMenu.add_command(label="Sair", command=self.quit)
        menubar.add_cascade(label="Ficheiro", menu=fileMenu)
        ...
        graphMenu = tk.Menu(menubar, tearoff=False)  # Gráficos de visualização
        graphMenu.add_command(label="Série original", command=self.G1)
        graphMenu.add_command(label="Série com identificação das falhas de curta e longa duração", command=self.G2)
        graphMenu.add_command(label="Série com falhas de curta duração preenchidas", command=self.G3)
        menubar.add_cascade(label="Visualizar gráficos", menu=graphMenu)
    ...
    def graph_original(self):  # Série de input
        ...
        self.ax.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                     ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax.legend()
        self.ax.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_graph_original(self, values, dates):  # Atualiza Gráfico original
        ...
        if len(nans) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            ...
            self.ax.axvline(x=i, color='k', linestyle='--', linewidth=0.3, label='Falhas', marker='|')  # legenda
    ...
    def table_info_nan(self):  # Tabela de contagem de falhas e medicoes, bem como quantos segundos para dividir os tipos
        tk.Label(self, text="Contagem", bg='white',
                 font=NORMAL_FONT).grid(column=95, row=0, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Medições", bg='white',  # Número de medições
                 font=NORMAL_FONT, anchor='e').grid(column=90, row=1, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Medições com falha", bg='white',  # Número de Falhas
    ...
    def graph_nas_separated_original(self):  # Gráfico de onde estao os nan
        ...
        self.ax2.set_title("Série com identificação das falhas de curta e longa duração")
        # we plot y as a function of a, which parametrizes x
        self.ax2.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                      ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax2.legend()
        self.ax2.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_graph_nas_separated(self):  # Atualiza o gráfico dos nan
        ...
        self.ax2.cla()
        self.ax2.plot(dates, values, '#4772FF', label='Caudal')
    ...
    def table_nas_separated_original(self):  # Tabela de contagens de nans
        tk.Label(self, text="Contagem", bg='white', font=NORMAL_FONT).grid(column=95, row=13, columnspan=5,
                                                                           sticky='NSWE')
        tk.Label(self, text="Falhas de curta duração", bg='white', font=NORMAL_FONT, anchor='e',
                 fg='green').grid(column=90, row=14, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Falhas de longa duração", bg='white', font=NORMAL_FONT, anchor='e', fg='red').grid(
            column=90, row=15, columnspan=5, sticky='NSWE')
    ...
    def final_graph_original(self):  # Gráfico com falhas curtas preenchidas
        ...
        self.ax3.set_title("Série com falhas de curta duração preenchidas")
        # we plot y as a function of a, which parametrizes x
        self.ax3.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                      ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax3.legend()
        self.ax3.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_final_graph(self):  # Atualiza gráfico final com falhas curtas imputadas
        ...
        if len(self.set_nans_long) > 0:
            ...
            self.ax3.axvline(x=dates[self.set_nans_long[i]], color='r', linestyle='--', linewidth=0.3,
                             label='Falhas de longa duração', marker='|')
    ...
    def table_inputation_original(self):  # Metodos de imputacao a escolher
        tk.Label(self, text="Método de imputação para falhas de curta duração", bg='white', font=NORMAL_FONT,
                 anchor='e', fg='black', wraplength=150).grid(column=90, row=40, columnspan=5, sticky='NSWE')
    ...
    def imputation(self):  # Métodos de imputacao a aplicar
        ...
        if imputation_option == 'Interpolação Linear':
            self.reconstructedData = df.astype(float).interpolate(method='linear')

        elif imputation_option == 'Interpolação Polinomial':
    ...
    def G1(self):  # Série original com na identificados
        ...
        if len(nans) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            ...
            plt.axvline(x=i, color='k', linestyle='--', linewidth=0.5, label='Falhas', marker='|')  # legenda
    ...
    def G2(self):  # Série original com Na's identificados como de curta ou longa duração
        try:
            ...
            plt.figure(2)
            plt.cla()
            plt.plot(dates, values, '#4772FF', label='Caudal')
            ...
            for row in self.set_nans:
                ...
                if diff > int(self.tNA.get()):  # Pinto de verde ou vermelho, consoante o thsold
                    ...
                    self.ax2.axvline(x=dates[i], color='#000000', linestyle='--', linewidth=0.3)  # Falhas Curtas
                    self.ax2.axvline(x=dates[i + 1], color='#000000', linestyle='--', linewidth=0.3)  # Falhas Curtas
            plt.title("Série com identificação das falhas de curta e longa duração")
            plt.gca().xaxis.set_major_formatter(DATE_FORM)
            plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
            plt.legend()
            plt.show()
            ...
            ax2.set_title("Série com identificação das falhas de curta e longa duração")
            # we plot y as a function of a, which parametrizes x
            ax2.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                     ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
            ax2.legend()
            ax2.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
            plt.show()
    ...
    def G3(self):  # série original com valores imputados e identificação de falhas de longa duração
        try:
            ...
            plt.plot(dates[self.set_nans_short], self.reconstructedData['value'][self.set_nans_short], "^g",
                     label='Falhas curtas preenchidas')
            ...
            plt.title("Série com falhas de curta duração preenchidas")
            plt.gca().xaxis.set_major_formatter(DATE_FORM)
            plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
            plt.legend()
            plt.show()
...
class Tab3(tk.Frame):  # Normalização temporal
    def __init__(self, parent):
        ...
        # Buttons
        self.Btn_Sep = tk.Button(self, text="Criar espaçamento uniforme e separar periodos sem medição",
                                 command=self.upd_graph_uniform_spacing,
                                 font=NORMAL_FONT, relief=GROOVE, wraplength=250).grid(column=92, row=11, columnspan=6,
                                                                                       sticky='NSWE')
        self.Btn_Spacing = tk.Button(self, text="Transformar em espaçamento uniforme ",
    ...
    def menubar(self, root):  # Menu igual ao Tab1 e Tab2, mudando os gráficos
        menubar = tk.Menu(root)
        fileMenu = tk.Menu(menubar, tearoff=False)
        fileMenu.add_command(label="Importar", command=lambda: root.file_input(Tab3))
        fileMenu.add_command(label="Limpar dados", underline=1, command=self.clear)
        fileMenu.add_command(label="Exportar", underline=1, command=lambda: file_output("dataset3"))
        fileMenu.add_separator()
        fileMenu.add_command(label="Sair", command=self.quit)
        menubar.add_cascade(label="Ficheiro", menu=fileMenu)
    ...
    def graph_original(self):  # Série original
        ...
        self.ax.plot(['00:00:00', '06:00:00',
                      '12:00:00', '18:00:00', '24:00:00'], ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax.legend()
        self.ax.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_graph_original(self, values, dates):  # Atualizacao da série original
        ...
        if len(self.nan) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            ...
            self.vline = self.ax.axvline(x=i, color='k', linestyle='--', linewidth=0.5, label='Falhas de longa duração',
                                         marker='|')  # legenda
    ...
    def table_info_nan(self):  # Tabela de medicoes e falhas
        tk.Label(self, text="Contagem", bg='white',
                 font=NORMAL_FONT).grid(column=95, row=0, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Medições", bg='white',
                 font=NORMAL_FONT, anchor='e').grid(column=90, row=1, columnspan=5, sticky='NSWE')
        tk.Label(self, text="Medições com falha", bg='white',
                 font=NORMAL_FONT, anchor='e').grid(column=90, row=2, columnspan=5, sticky='NSWE')

        tk.Label(self, text="Espaçamento (s)", bg='white',
                 font=NORMAL_FONT, anchor='e').grid(column=90, row=3, columnspan=5, sticky='NSWE')

        tk.Label(self, text="Periodo longo (s)", bg='white',
    ...
    def graph_uniform_spacing_original(self):  # Gráfico da previsao do espacamento normalizado
        ...
        self.fig2 = Figure(figsize=(1, 1), dpi=50)
        self.ax2 = self.fig2.add_subplot(111)
        self.ax2.set_title("Série com identificação do espaçamento uniforme")
    ...
    def upd_graph_uniform_spacing(self):  # Atualiza gráfico do espacamento temporal
        ...
        if len(self.nan) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            self.vline = self.ax2.axvline(x=i, color='k', linestyle='--', linewidth=0.3, label='Falhas de longa duração',
                                          marker='|')  # legenda
        ...
        self.vline = self.ax2.axvline(x=i, color='r', linestyle='--', linewidth=0.3, label='Espaçamento uniforme',
                                      marker='|')
        ...
        if check:
            self.vline = self.ax2.axvline(x=dates[lst_vlue], color='indigo', linestyle='--', linewidth=1,
                                          label='Periodo sem medição',
                                          marker='|')  # Falhas Curtas
    ...
    def table_uniform_spacing(self):  # Tabela com informações sobre o processo de normalização temporal
        ...
        tk.Label(self, text="Periodos sem medição", bg='white',
    ...
    def final_graph_original(self):  # Gráfico final com a normalizacao feita
        self.fig3 = Figure(figsize=(1, 1), dpi=50)
        self.ax3 = self.fig3.add_subplot(111)
        self.ax3.set_title("Série com identificação das falhas de curta e longa duração")
        # we plot y as a function of a, which parametrizes x
        self.ax3.plot(['00:00:00', '06:00:00', '12:00:00', '18:00:00', '24:00:00'],
                      ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax3.legend()
    ...
    def upd_final_graph(self):  # Atualiza o gráfico final
        ...
        self.ax3.cla()
        self.ax3.plot(dates, values, '#4772FF', label='Série original')
        self.ax3.plot(self.mergin['date'], self.mergin['value'], marker="o", color='red', label='Série normalizada temporalmente')
        ...
        if len(nans) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            ...
            self.vline = self.ax3.axvline(x=i, color='grey', linestyle='--', linewidth=0.5,
                                          label='Falhas de longa duração', marker='|')  # legenda

        # format graph
        self.ax3.xaxis.set_major_formatter(DATE_FORM)
        self.ax3.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
        self.ax3.set_title("Série com identificação do espaçamento uniforme")
    ...
    def G1(self):  # Série original com na identificados
        ...
        if len(self.nan) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            ...
            for i in dates[self.nan]:
                plt.axvline(x=i, color='k', linestyle='--', linewidth=0.3)
            plt.axvline(x=i, color='k', linestyle='--', linewidth=0.5, label='Falhas de longa duração', marker='|')  # legenda
    ...
    def G2(self):  # Série original com Na's identificados como de curta ou longa duração
        ...
        plt.axvline(x=i, color='r', linestyle='--', linewidth=0.3, label='Espaçamento uniforme', marker='|')
        ...
        for i in range(len(time_spacing[0])):
            if int(time_spacing[0][i]) > int(self.tPL.get()) and math.isnan(values[i]) is not True:
                ...
                lst_vlue = i
        if check:
            plt.axvline(x=dates[lst_vlue], color='indigo', linestyle='--', linewidth=1, label='Periodo sem medição',
                        marker='|')  # Falhas Curtas
    ...
    def G3(self):  # série original com valores imputados e identificação de falhas de longa duração
        ...
        plt.plot(dates, values, '#4772FF', label='Série original')
        plt.plot(self.mergin['date'], self.mergin['value'], marker="o", color='red', label='Série normalizada temporalmente')
...
class Tab4(tk.Frame):  # Preenchimento de falhas (WIP)
    def __init__(self, parent):
        ...
        self.Btn_Import = tk.Button(self, text="Importar Histórico", command=self.file_input,
                                  font=NORMAL_FONT, relief=GROOVE).grid(row=2, column=92, columnspan=6,
                                                                        sticky='NSEW')

        self.Btn_Decompose = tk.Button(self, text="Decomposição do Histórico", command=self.btn_series_decomposition,
                                  font=NORMAL_FONT, relief=GROOVE).grid(row=18, column=92, columnspan=6,
                                                                        sticky='NSEW')

        self.Btn_Reconstruction = tk.Button(self, text="Preenchimento de falhas", command=self.btn_reconstruction,
    ...
    def menubar(self, root):
        menubar = tk.Menu(root)
        fileMenu = tk.Menu(menubar, tearoff=False)
        fileMenu.add_command(label="Importar", command=lambda: root.file_input(Tab4))
        fileMenu.add_command(label="Limpar dados", underline=1, command=self.clear)
        fileMenu.add_command(label="Exportar", underline=1, command=lambda: file_output("dataset4"))
        fileMenu.add_separator()
        fileMenu.add_command(label="Sair", command=self.quit)
        menubar.add_cascade(label="Ficheiro", menu=fileMenu)
        ...
        methodMenu = tk.Menu(menubar, tearoff=False)
        methodMenu.add_command(label="Identificação de falhas", command=lambda: root.show_frame(Tab1, from_menu=True))
        methodMenu.add_command(label="Imputação de valores pontuais",command=lambda: root.show_frame(Tab2, from_menu=True))
        methodMenu.add_command(label="Normalização temporal", command=lambda: root.show_frame(Tab3, from_menu=True))
        methodMenu.add_command(label="Preenchimento de falhas", command=lambda: root.show_frame(Tab4, from_menu=True))
        menubar.add_cascade(label="Métodos de tratamento", menu=methodMenu)

        graphMenu = tk.Menu(menubar, tearoff=False)  # Gráficos de visualização
        graphMenu.add_command(label="Série original / histórico", command=self.G1)
        graphMenu.add_command(label="Boxplot ao nível de cada timestamp", command=self.G2)
        graphMenu.add_command(label="Boxplot dia de semana / fim-de-semana", command=self.G3)
        graphMenu.add_command(label="Boxplot ao nível de cada dia", command=self.G4)
        graphMenu.add_command(label="Boxplot ao nível de cada dia da semana", command=self.G5)
        graphMenu.add_command(label="Decomposição de sazonalidade", command=self.G6)
        graphMenu.add_command(label="Série tratada", command=self.G7)
        menubar.add_cascade(label="Visualizar gráficos", menu=graphMenu)
    ...
    def graph_original(self):  # Série de input
        self.fig = Figure(figsize=(1, 1), dpi=50)
        self.ax = self.fig.add_subplot(111)
        self.ax.set_title("Série original / histórico")
        # we plot y as a function of a, which parametrizes x
        self.ax.plot(['00:00:00', '06:00:00',
                      '12:00:00', '18:00:00', '24:00:00'], ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax.legend()
    ...
    def table_info_measures(self):
        tk.Label(self, text="Contagem", bg='white', font=NORMAL_FONT).grid(column=94, row=4, columnspan=3,
                                                                           sticky='NSWE')
        tk.Label(self, text="Espaçamento (s)", bg='white', font=NORMAL_FONT).grid(column=97, row=4, columnspan=3,
                                                                           sticky='NSWE')

        tk.Label(self, text="Em validação", bg='white', font=NORMAL_FONT, anchor='e',
                 fg='red').grid(column=90, row=5, columnspan=4, sticky='NSWE')
        tk.Label(self, text="Histórico", bg='white', font=NORMAL_FONT, anchor='e', fg='#257AB5').grid(
            column=90, row=6, columnspan=4, sticky='NSWE')
        tk.Label(self, text="Em falta", bg='white', font=NORMAL_FONT, anchor='e', fg='black').grid(
    ...
    def upd_graph_original(self, values, dates):  # Atualiza o gráfico inicial
        ...
        self.ax.cla()
        self.ax.plot(dates, values, marker="o", color='red', label='Série normalizada temporalmente')
        ...
        if len(nans) != 0:  # Sem este if, o programa rebenta por não ter nan's para fazer a mancha
            ...
            self.vline = self.ax.axvline(x=dates[nans[i]], color='grey', linestyle='--', linewidth=0.5,
                                          label='Falhas por preencher', marker='|')  # legenda
    ...
    def upd_graph_original2(self, values, dates, values_h, dates_h):  # Atualiza o gráfico inicial
        ...
        self.ax.plot(dates_h, values_h, marker="^", color='#257AB5', label='Histórico')
    ...
    def boxplot_hour(self):  # Série de input
        ...
        self.ax2.set_title("Boxplot ao nível de cada timestamp")
        self.ax2.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def boxplot_wdwk(self):  # Série de input
        ...
        self.ax3.set_title("Boxplot ao nível de cada timestamp // Dia semana e fim-de-semana")
        self.ax3.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_boxplot_wdwk(self, df):
        ...
        self.ax3.legend(handles, ["Dia de semana", "Fim-de-semana"])
    ...
    def boxplot_day(self):  # Série de input
        ...
        self.ax4.set_title("Boxplot ao nível de cada dia")
        self.ax4.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def boxplot_weekday(self):  # Série de input
        ...
        sns.boxplot(x=None, y=None, data=None, ax=self.ax3)
        self.ax5.set_title("Boxplot ao nível de cada dia da semana")
        self.ax5.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def graph_seasonal_decomp(self):
        ...
        self.ax61.set_title("Decomposição de sazonalidade")
    ...
    def table_info_season(self):
        ...
        #                                                                    sticky='NSWE')
        tk.Label(self, text="Periodo de sazonalidade ", bg='white', font=NORMAL_FONT, fg='black', anchor='e').grid(column=90, row=15, rowspan=1, columnspan=7, sticky='NSWE')
        tk.Label(self, text="Método de decomposição ", bg='white', font=NORMAL_FONT, fg='black', anchor='e').grid(column=90, row=16, rowspan=1, columnspan=7, sticky='NSWE')
    ...
    def graph_forecast(self):  # Série de input
        ...
        self.ax7.set_title("Série com falhas preenchidas")
        # we plot y as a function of a, which parametrizes x
        self.ax7.plot(['00:00:00', '06:00:00',
                      '12:00:00', '18:00:00', '24:00:00'], ['N/A', 'N/A', 'N/A', 'N/A', 'N/A'], 'white', label='Caudal')
        self.ax7.legend()
        self.ax7.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
    ...
    def upd_graph_forecast(self, history,dates):
        ...
        self.ax7.plot(trated_d, treated_v, marker="^", color='#257AB5', label='Série tratada')
        # self.ax7.plot(dates, values, marker="o", color='red', label='Série original')
        ...
        # format graph
        self.ax7.set_ylabel(r'Caudal ($m^{3}h^{-1}$)')
        self.ax7.set_title("Série original")
    ...
    def G1(self):
        ...
        plt.plot(dates, values, marker = 'o', color='red', label='Série normalizada temporalmente')
        plt.plot(dates_h, values_h, marker="^", color='#257AB5', label='Histórico')
        ...
        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.title("Série original / histórico")
        plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
        plt.legend()
        plt.show()
    ...
    def G2(self):
        ...
        plt.title("Boxplot ao nível de cada timestamp")
    ...
    def G3(self):
        ...
        ax.legend(handles, ["Dia de semana", "Fim-de-semana"])
        ax.set_xticklabels(ax.get_xticklabels(), rotation=45)

        plt.title("Boxplot ao nível de cada timestamp // Dia semana e fim-de-semana")
    ...
    def G4(self):
        ...
        plt.title("Boxplot ao nível de cada dia")
    ...
    def G5(self):
        ...
        plt.title("Boxplot ao nível de cada dia da semana")
    ...
    def G6(self):
        ...
        plt.title("Decomposição de sazonalidade")
        plt.plot(x, observed)
    ...
    def G7(self):  # série original com valores imputados e identificação de falhas de longa duração
        ...
        plt.plot(dates_h, values_h, marker="^",
                 color='#257AB5', label='Série tratada')

        plt.plot(dates, values, marker="o",
                 color='red', label='Série original')

        plt.gca().xaxis.set_major_formatter(DATE_FORM)
        plt.title("Série tratada")
        plt.ylabel(r'Caudal ($m^{3}h^{-1}$)')
        plt.legend()
        plt.show()
